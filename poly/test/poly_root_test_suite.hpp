...
class poly_root_test_suite : public Test::Suite
{
  ...
  private:
    void descartes_test()
    {
      ...
      {
        typename eli::poly::polynomial<data__>::coefficient_type coef(6), coef_out, coef_ref;

        // set coefficients
        coef << 2, -10, -20, 0, 5, 1;

        eli::poly::polynomial<data__> f(coef);

        TEST_ASSERT(eli::poly::root::descartes_rule(f)==5);
        TEST_ASSERT(eli::poly::root::descartes_rule(f, true)==2);
        TEST_ASSERT(eli::poly::root::descartes_rule(f, false)==3);
      }

      // this is case where know the Sturm functions and the real roots, -1 and 1. The
      // other two roots are complex conjugates.
      {
        typename eli::poly::polynomial<data__>::coefficient_type coef(5), coef_out, coef_ref;
        ...
        TEST_ASSERT(eli::poly::root::descartes_rule(f)==4);        // should be 2 but extra roots reported on neg. side
        TEST_ASSERT(eli::poly::root::descartes_rule(f, true)==1);
        TEST_ASSERT(eli::poly::root::descartes_rule(f, false)==3); // should be 1 but extra roots reported
      }

      // this is case with a manually built polynomial with no repeated roots
      {
        data__ roots[7] = {-6, -4, -2, 2, 4, 6, 8};
        eli::poly::polynomial<data__> f(roots, roots+7);

        TEST_ASSERT(eli::poly::root::descartes_rule(f)==7);
        TEST_ASSERT(eli::poly::root::descartes_rule(f, true)==4);
        TEST_ASSERT(eli::poly::root::descartes_rule(f, false)==3);
      }
      ...
      // -4, 2 and 5 as roots as well
      {
        ...
        eli::poly::polynomial<data__> f(roots, roots+5);
      ...
      // -4, 2 and 5 as roots as well
      {
        ...
        eli::poly::polynomial<data__> f(roots, roots+6);

        TEST_ASSERT(eli::poly::root::descartes_rule(f)==6);
        TEST_ASSERT(eli::poly::root::descartes_rule(f, true)==2);
        TEST_ASSERT(eli::poly::root::descartes_rule(f, false)==4);
      }
      ...
      // 2 (twice) with -4 and 5 as roots as well
      {
        ...
        TEST_ASSERT(eli::poly::root::descartes_rule(f)==7);
        TEST_ASSERT(eli::poly::root::descartes_rule(f, true)==3);
        TEST_ASSERT(eli::poly::root::descartes_rule(f, false)==4);
      }
    ...
    void sturm_test()
    {
      ...
      // the roots are between (-4, -3), (-3, -2), (-1, 0), (0, 1), (1, 2)
      {
        ...
        eli::poly::polynomial<data__> f(coef);
        std::vector< eli::poly::polynomial<data__> > p;

        // get the Sturm functions and manipulate to match reference values
        eli::poly::root::sturm_functions(p, f);
        p[3].multiply(3);
        p[4].multiply(17);
        ...
        TEST_ASSERT(eli::poly::root::sturm_count(p, -5,  5)==5); // total real roots
        TEST_ASSERT(eli::poly::root::sturm_count(p, -5,  0)==3); // negative real roots
        TEST_ASSERT(eli::poly::root::sturm_count(p,  0,  5)==2); // positive real roots
        TEST_ASSERT(eli::poly::root::sturm_count(p, -4, -3)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p, -3, -2)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p, -1,  0)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  0,  1)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  1,  2)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p, -2, -1)==0); // no root

        TEST_ASSERT(eli::poly::root::sturm_count(p)==5);         // total number of real roots
        TEST_ASSERT(eli::poly::root::sturm_count(p, false)==3);  // total number of negative real roots
        TEST_ASSERT(eli::poly::root::sturm_count(p, true)==2);   // total number of positive real roots
      }
      ...
      // other two roots are complex conjugates.
      {
        ...
        // calculate the root count and get back the Sturm functions
        TEST_ASSERT(eli::poly::root::sturm_count(f, -5,  5)==2); // total real roots
        TEST_ASSERT(eli::poly::root::sturm_count(f, -5,  0)==1); // negative real roots
        TEST_ASSERT(eli::poly::root::sturm_count(f,  0,  5)==1); // positive real roots
        TEST_ASSERT(eli::poly::root::sturm_count(f, -2,  0)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(f,  0,  2)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(f, -3, -2)==0); // no root

        TEST_ASSERT(eli::poly::root::sturm_count(f)==2);         // total number of real roots
        TEST_ASSERT(eli::poly::root::sturm_count(f, false)==1);  // total number of negative real roots
        TEST_ASSERT(eli::poly::root::sturm_count(f, true)==1);   // total number of positive real roots
      }
      ...
      // this is case with a manually built polynomial with no repeated roots
      {
        ...
        // calculate the root count and get back the Sturm functions
        TEST_ASSERT(eli::poly::root::sturm_count(p, -9,  9)==7); // total real roots
        TEST_ASSERT(eli::poly::root::sturm_count(p, -9,  0)==3); // negative real roots
        TEST_ASSERT(eli::poly::root::sturm_count(p,  0,  9)==4); // positive real roots
        TEST_ASSERT(eli::poly::root::sturm_count(p, -7, -5)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p, -5, -3)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p, -3, -1)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  1,  3)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  3,  5)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  5,  7)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  7,  9)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p, -8, -7)==0); // no root

        // switched to passing the polynomial for no performance reason, just testing variety
        TEST_ASSERT(eli::poly::root::sturm_count(f)==7);         // total number of real roots
        TEST_ASSERT(eli::poly::root::sturm_count(f, false)==3);  // total number of negative real roots
        TEST_ASSERT(eli::poly::root::sturm_count(f, true)==4);   // total number of positive real roots
      }
      ...
      // -4, 2 and 5 as roots as well
      {
        ...
        // calculate the root count and get back the Sturm functions
        TEST_ASSERT(eli::poly::root::sturm_count(p, -9,  9)==4); // total real roots
        TEST_ASSERT(eli::poly::root::sturm_count(p, -9,  0)==2); // negative real roots
        TEST_ASSERT(eli::poly::root::sturm_count(p,  0,  9)==2); // positive real roots
        TEST_ASSERT(eli::poly::root::sturm_count(p, -5, -3)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p, -3, -1)==1); // double root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  1,  3)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  4,  6)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  3,  4)==0); // no root

        // switched to passing the polynomial for no performance reason, just testing variety
        TEST_ASSERT(eli::poly::root::sturm_count(f)==4);         // total number of real roots
        TEST_ASSERT(eli::poly::root::sturm_count(f, false)==2);  // total number of negative real roots
        TEST_ASSERT(eli::poly::root::sturm_count(f, true)==2);   // total number of positive real roots
      }
      ...
      // 2 (twice) with -4 and 5 as roots as well
      {
        ...
        TEST_ASSERT(eli::poly::root::sturm_count(p,  0,  9)==2); // positive real roots
        TEST_ASSERT(eli::poly::root::sturm_count(p, -5, -3)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p, -3, -1)==1); // triple root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  1,  3)==1); // double root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  4,  6)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  3,  4)==0); // no root
      ...
      // 5 (thrice) with -4 and 2 as roots as well
      {
        ...
        TEST_ASSERT(eli::poly::root::sturm_count(p, -3, -1)==1); // double root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  1,  3)==1); // root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  4,  6)==1); // triple root bracket
        TEST_ASSERT(eli::poly::root::sturm_count(p,  3,  4)==0); // no root
    ...
    {
      eli::poly::polynomial<data__> f;

      typename eli::poly::polynomial<data__>::coefficient_type a;
      int nroot;
      ...
      nroot=eli::poly::root::closed_form(root, a.data(), 0);
    ...
    void closed_form_root_test_1()
    {
      ...
      f.get_coefficients(a);
      nroot=eli::poly::root::closed_form(root, a.data(), deg);
      TEST_ASSERT(nroot==0);
      TEST_ASSERT(root.size()==0);
