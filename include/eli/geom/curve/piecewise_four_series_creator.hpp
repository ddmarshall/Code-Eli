...
#ifndef eli_geom_curve_piecewise_four_series_creator_hpp
#define eli_geom_curve_piecewise_four_series_creator_hpp
  ...
  namespace geom
  {
    namespace airfoil
    {
      ...
      class four_series
      {
        ...
        public:
          four_series() : thickness(00), camber(0), camber_loc(0), sharp_te(false)
          {
            recalc_params();
            recalc_coefficients();
          }

          four_series(const four_series<data_type> &fs)
        ...
        protected:
          ...
          void recalc_coefficients()
          {
            ...
            calc_four_series_args(coef_mat, 0, static_cast<data_type>(1));        // (1) trailing edge location
            calc_four_series_der_args(coef_mat, 1, static_cast<data_type>(1));    // (2) trailing edge slope
            calc_four_series_args(coef_mat, 2, static_cast<data_type>(0.1));      // (3) leading edge shape
            calc_four_series_args(coef_mat, 3, static_cast<data_type>(0.3));      // (4) thickness at x/c=0.3 (should be max thickness location, but isn't)
            calc_four_series_der_args(coef_mat, 4, static_cast<data_type>(0.3));  // (5) slope at x/c=0.3 (should be zero slope at max thickness, but isn't)
          ...
          static void calc_four_series_args(Eigen::MatrixBase<Derived1> &A, const typename Derived1::Index &i, const data_type &xi)
          {
          ...
          template<typename Derived1>
          static void calc_four_series_der_args(Eigen::MatrixBase<Derived1> &A, const typename Derived1::Index &i, const data_type &xi)
          {
...
namespace eli
{
  namespace geom
  {
    ...
    {
      template<typename data__, unsigned short dim__, typename tol__>
      class piecewise_four_series_creator : public piecewise_airfoil_creator_base<data__, dim__, tol__>
      {
        public:
          ...
          piecewise_four_series_creator() : piecewise_airfoil_creator_base<data_type, dim__, tolerance_type>(2, 0) {}
          piecewise_four_series_creator(const piecewise_four_series_creator<data_type, dim__, tolerance_type> &ppc)
            : piecewise_airfoil_creator_base<data_type, dim__, tolerance_type>(ppc) {}
          ~piecewise_four_series_creator() {}
